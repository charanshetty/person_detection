...
--uses getmeans.lua to  calculate the global average
----------------------------------------------------------------------
...
for k = 1, totSize, 1 do
  ...
  for t = 1,testData:size(),opt.batchSize do
     ...
     for i = 1,opt.batchSize do
        ...
        if (((((preds[i])[1]>(preds[i])[2]) and (targets[i]==1)) and preds[1][1]-preds[1][2] > 14)) then
          ...
          for _,b in ipairs(boundingBox) do
            tfile = 'Scaledbox_v5/'.. tostring(ls(testDir)[p]) ..tostring(k)..'.png'
            print("here")
            ...
            if not(tmp2) then
              tmp = img:clone()
              --Oimg = img:clone()
              img[{ 2,b.yMin,{b.xMin,b.xMax} }] = 1
              img[{ 2,b.yMax,{b.xMin,b.xMax}}] = 1
              img[{ 2,{b.yMin,b.yMax}, b.xMin}] = 1
              img[{ 2,{b.yMin,b.yMax}, b.xMax}] = 1
              im=image.scale(img,Oimg:size(3),Oimg:size(2))
              image.save(tfile,im)
              tmp2=img:clone()
              img = tmp:clone()
              else
              ...
              print(tfile, created)
              im=image.scale(tmp2,Oimg:size(3),Oimg:size(2))
              image.save(tfile,im)
              img = tmp:clone()
            end
          end
